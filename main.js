...
function toDecimal(numberStr, base) {
  ...
  for (let i = 0; i < numberStr.length; i++) {
    ...
    if (val >= base) {
      throw new Error(
        `Číslice '${numberStr[i]}' s hodnotou ${val} není platná v soustavě o základu ${base}.`
      );
    }
...
export function main(inputNumber, inputNumberSystem, outputNumberSystem) {
  // Krok 1: Převod ze vstupní soustavy do desítkové pomocí Hornerova schématu
  const decimalValue = toDecimal(inputNumber, inputNumberSystem);

  // Krok 2: Převod z desítkové do výstupní soustavy opakovaným dělením
